 ...
 * injected `MintFunction`, so every test builds a fresh client and queues
 * `MintOutcome`s on a mock mint.
 */
...
describe("getSession", () => {
  ...
  it("clears the in-flight slot after a failed mint so the next call retries", async () => {
    ...
    // The refusal is named, not rethrown raw.
    await expect(client.getSession()).rejects.toMatchObject({
      kind: "mint_refused",
      status: 500,
    });
