...
describe("buildBusinessesUrl", () => {
  it("sends the trimmed name and the limit, and nothing else", () => {
    ...
    // React hook passes the console's 5.
    expect(buildBusinessesUrl(BASE_URL, { q: " osir ", limit: 5 })).toBe(
      "https://api.test/autocomplete/businesses?q=osir&limit=5",
    );
  ...
  it("leaves the limit to the tier when none is given", () => {
    ...
    expect(buildBusinessesUrl(BASE_URL, { q: "osir" })).toBe(
      "https://api.test/autocomplete/businesses?q=osir",
    );
  });
    ...
    const url = new URL(
      buildBusinessesUrl(BASE_URL, {
        ...
        filters: {
          ...
          person: { role: "officer", name: "Timothy Hyde" },
          domicileState: "DE",
          state: ["DE", "CA"],
        },
      }),
  ...
  it("joins include into one parameter", () => {
    const url = new URL(
      buildBusinessesUrl(BASE_URL, { q: "osir", include: ["people"] }),
    );
  ...
  it("leaves out a filter with nothing in it", () => {
    ...
    ).toBe("https://api.test/autocomplete/businesses?q=osir");
  });
    ...
    expect(
      buildBusinessesUrl(
        BASE_URL,
        {
          ...
          limit: 5,
          include: ["people"],
          filters: { state: ["DE"], person: { name: "Hyde" } },
        },
        { withFilters: false },
      ),
    ).toBe(
  ...
  it("does not double the slash of a base URL that ends in one", () => {
    expect(buildBusinessesUrl("https://api.test/", { q: "osir" })).toBe(
      "https://api.test/autocomplete/businesses?q=osir",
    );
  });
});
...
describe("the tier's measure of a query", () => {
  it("depuncts the way the tier does", () => {
    expect(depunct("Osiris & Co.")).toBe("osiris and co");
  ...
  it("measures the stem on the stripped, depuncted query", () => {
    expect(stemLength("Osiris & Co.")).toBe("osiris and co".length);
...
describe("client.suggest", () => {
  it("sends the grant header and reads the index the tier answered from", async () => {
    ...
    const result = await client.suggest({ q: "osir", limit: 5 });

    expect(fetchImpl).toHaveBeenCalledTimes(1);
    const [url, init] = fetchImpl.mock.calls[0] ?? [];
    expect(url).toBe("https://api.test/autocomplete/businesses?q=osir&limit=5");
    expect(init?.headers[SESSION_HEADER]).toBe("grant-1");
    expect(init?.credentials).toBe("omit");
    expect(result.response.suggestions[0]?.label).toBe("OSIRIS RATINGS, INC.");
  ...
  it("re-mints once on a 401 and replays with the new grant", async () => {
    ...
    const result = await client.suggest({ q: "osir" });
  ...
  it("re-mints when the tier refused the session for changing its query too often", async () => {
    ...
    await client.suggest({ q: "osiris ratings" });
  ...
  it("waits out Retry-After on rate_limited, then replays with the same grant", async () => {
    ...
    const pending = client.suggest({ q: "osir" });
  ...
  it("gives up during the wait when the keystroke is superseded mid-wait", async () => {
    ...
    const aborted = expect(
      client.suggest({ q: "osir" }, { signal: controller.signal }),
  ...
  it("stops on a spent coverage budget without retrying", async () => {
    ...
    const failure = client.suggest({ q: "osir" });
  ...
  it("recovers at most once", async () => {
    ...
    await expect(client.suggest({ q: "osir" })).rejects.toMatchObject({
      kind: "request_failed",
      status: 401,
    });
    expect(fetchImpl).toHaveBeenCalledTimes(2);
  ...
  describe("the brake on a tier that keeps refusing fresh grants", () => {
    ...
    it("stops minting after consecutive calls that failed even after a re-mint", async () => {
      ...
      const braked = client.suggest({ q: "osiri" });
      ...
      await expect(client.suggest({ q: "osiris" })).rejects.toMatchObject({
        kind: "request_failed",
        status: 401,
        reason: null,
      });
    ...
    it("keeps the brake to the client that armed it, until a reset", async () => {
      ...
      for (let call = 0; call < AUTH_FAILURES_BEFORE_BRAKE; call += 1) {
        await expect(braked.client.suggest({ q: "osir" })).rejects.toThrow();
      }
      await expect(braked.client.suggest({ q: "osir" })).rejects.toMatchObject({
        kind: "auth_braked",
      });

      // The console's brake was per tab; a second client starts clean.
      const other = setup({ now: clock.now });
      other.fetchImpl.mockResolvedValue(reply(200, okBody));
      await expect(other.client.suggest({ q: "osir" })).resolves.toBeDefined();

      // And a sign-out lifts it on the client that armed it.
      braked.client.reset();
      braked.fetchImpl.mockResolvedValue(reply(200, okBody));
      await expect(braked.client.suggest({ q: "osir" })).resolves.toBeDefined();
    });
    ...
    it("does not count a rate limit waited out and then followed by an expiry", async () => {
      ...
      await expect(client.suggest({ q: "osiris" })).resolves.toMatchObject({
    ...
    it("forgets the failures once a call succeeds", async () => {
      ...
      await expect(client.suggest({ q: "osiri" })).resolves.toBeDefined();
    ...
    it("does not count a rate limit or a validation error as an auth failure", async () => {
      ...
      await expect(client.suggest({ q: "osiris" })).resolves.toBeDefined();
    });
  });
  ...
  describe("the filter stem", () => {
    ...
    it("withholds the filters from a query shorter than the grant's stem, and says so", async () => {
      ...
      const result = await client.suggest({ q: "osi", filters });

      const { url } = sentAt(fetchImpl, 0);
      expect(url.searchParams.get("q")).toBe("osi");
    ...
    it("sends the filters once the stem reaches the grant's", async () => {
      ...
      const result = await client.suggest({ q: "osir", filters });
    ...
    it("refuses before any fetch under onShortStem: throw", async () => {
      ...
      const failure = client.suggest({ q: "osi", filters });
  ...
  describe("a query the tier would refuse", () => {
    it.each<[string, Query]>([
      ...
      ["limit 0", { q: "osir", limit: 0 }],
      ["limit 21", { q: "osir", limit: 21 }],
      ["a fractional limit", { q: "osir", limit: 2.5 }],
    ...
    it("sends the bounds themselves", async () => {
      ...
      await client.suggest({ q: "osir", limit: 1 });
      await client.suggest({ q: "osir", limit: 20 });
  ...
  describe("what the client reports", () => {
    ...
    it("emits one request event per call, counting the requests since the mint", async () => {
      ...
      expect(events).toEqual([
        ...
        expect.objectContaining({ q: "osiri", requestsSinceMint: 2 }),
      ]);
    ...
    it("starts the count again on a re-mint, and counts the pivots refused", async () => {
      ...
      await client.suggest({ q: "osir" });
      await client.suggest({ q: "baselayer" });
    ...
    it("counts a refused request in the snapshot and tells subscribers", async () => {
      ...
      await expect(client.suggest({ q: "osiri" })).rejects.toMatchObject({
        status: 422,
      });
