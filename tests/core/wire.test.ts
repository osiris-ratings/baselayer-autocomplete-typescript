...
// The wire shape of `GET /autocomplete/businesses` as the Rust tier serves it
...
describe("a business suggestion", () => {
  ...
  it("reads an absent optional key as null, as the published contract allows", () => {
    ...
    const sparse = {
      ...
      related: {
        people: {
          truncated: false,
          items: [{ type: "person", label: "Timothy Hyde", matched: true }],
        },
  ...
  it("drops a raw id a rolled-back tier still sends, and reads no handle", () => {
    ...
    expect(item?.label).toBe("600 W Main St, Jefferson City, MO 65101");
  });
      ...
      parseSuggestion({
        ...
        related: {
          ...
          people: {
            ...
            items: [{ type: "person", label: "Timothy Hyde" }],
          },
        },
      }),
  ...
  it("refuses a missing relation, a fractional count and a body that is not an object", () => {
    ...
    for (const body of [null, "osir", [], 200]) {
