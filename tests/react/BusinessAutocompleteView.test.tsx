...
function renderTypeahead(
  ...
  fireEvent.change(utils.getByTestId("name"), { target: { value: "cinde" } });
...
describe("BusinessAutocompleteView", () => {
  it("keeps the caret where a character was typed into the middle of the name", () => {
    ...
    // and the caret lands at the end: a character typed into `CIN|RIS` reads
    // as typed at the end. So an owner exactly like the form's field.
    function Owner() {
      const [value, setValue] = useState("CINDER");
      return (
        <BusinessAutocompleteView
          id="businessName"
          value={value}
  ...
  it("reports typing to the parent and opens on it", () => {
    ...
    expect(props.onInputChange).toHaveBeenCalledWith("cinde");
  ...
  it("puts the name and the alternative name that matched on the first line", () => {
    ...
    expect(first).toHaveTextContent("CINDER RIGGING, INC.");
    expect(first).toHaveTextContent("also EMBERLINE");
  });
  ...
  it("puts the primary address and the officers on the second line", () => {
    ...
    expect(officers).toHaveTextContent("Wesley Crane +1");
  ...
  it("counts the officers beyond the head in the +N, not just the head", () => {
    ...
    ).toHaveTextContent("Wesley Crane +11");
  });

  it("hands the picked suggestion to the parent", () => {
    const { props } = renderTypeahead();

    fireEvent.click(screen.getByText("CINDER RACING STABLES, LLC"));
...
// The menu shell is part of the view: `data-testid` and `data-open`, hidden
...
describe("the menu", () => {
  ...
  it("follows the input's width unless told otherwise", () => {
    heldOpen();
  ...
  it("keeps a width of its own when it is not to follow the input", () => {
    heldOpen({ menuFollowsInputWidth: false });
  ...
  it("stays open through a blur and Escape while held open", () => {
    ...
    fireEvent.change(input, { target: { value: "cinde" } });
...
const cinderAltMarked: BusinessSuggestion = {
  ...cinder,
  highlight: [{ text: "EMBERLINE", matched: true }],
};
...
describe("the match marks", () => {
  ...
  it("marks the alternative name when that is what matched, and leaves the label alone", () => {
    renderTypeahead({ suggestions: [cinderAltMarked], found: 1 });

    const [row] = screen.getAllByTestId("business-suggestion");
    expect(marksOf(row)).toEqual(["EMBERLINE"]);
  ...
  it("marks nothing on a row the query reached some other way", () => {
    ...
    expect(first).toHaveTextContent("CINDER RACING STABLES, LLC");
    expect(marksOf(second)).toEqual([]);
    expect(second).toHaveTextContent("CINDER RIGGING, INC.");
    expect(second).toHaveTextContent("also EMBERLINE");
  });
  ...
  it("marks every typed word of a two-word match, whichever region", () => {
    ...
    const harbor: BusinessSuggestion = {
      ...
      highlight: [
        { text: "HARBOR CONCRETE", matched: true },
  ...
  it("paints the background highlight in the look's own color", () => {
    // The view hands the color to its stylesheet as a variable on the root,
    // and says a color is staged.
    const { root } = renderTypeahead({
      suggestions: [stableMarked],
      found: 1,
      look: {
  ...
  it("paints the menu, the lines and the squares in the look's colors", () => {
    // The view sets one variable per color on the root, which its classes
    // read.
    const { root } = renderTypeahead({
      suggestions: [cinder],
      found: 1,
...
describe("the people line", () => {
  it("marks a registered agent on the row so it is not read as a person", () => {
    ...
    expect(line).toHaveTextContent("NORTHGATE AGENT SERVICES, INC +1 · agent");
...
describe("the host's classes", () => {
  ...
  it("emits no bl-ac classes when unstyled, and keeps every data attribute", () => {
    const { container } = openDefault(
      baseProps({
        unstyled: true,
        suggestions: [stableMarked, cinder],
  ...
  it("draws the host's words over the defaults, one at a time", () => {
    openDefault(
      baseProps({
        ...
        suggestions: [cinder, stable, agentsOnly],
        found: 3,
      }),
    );
    ...
    ).toHaveTextContent("Wesley Crane and 1 more");
    expect(
      third!.querySelector('[data-testid="business-suggestion-officers"]'),
    ).toHaveTextContent("NORTHGATE AGENT SERVICES, INC and 1 more (agent)");
    expect(screen.getByTestId("business-suggestions-footer")).toHaveTextContent(
      "3 businesses",
    );
  });
