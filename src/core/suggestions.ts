...
/**
 * The typed query as the tier tokenizes it for the marks — `normalize_name`,
 * then `fold`: trimmed and lower-cased, `&` as the word `and`, `-` and `_` as
 * spaces, every character that is not a letter, a digit or an apostrophe
 * dropped, diacritics folded. `Osi & Rat-ings, José` is `osi and rat ings
 * jose`.
 */
 ...
 * between two marked words into one part (`OSIRIS RATINGS` for `osi rat`), so
