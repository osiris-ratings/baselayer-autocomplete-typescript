...
/** How many requests the page has made, for Debug's tab. */
export function NetworkCount({ log }: { log: NetworkLog }) {
  const all = useSyncExternalStore(log.subscribe, log.getSnapshot);
  return <span className="debug-count">{all.length}</span>;
}
