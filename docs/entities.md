...
| Route                      | Row type             | Relations it expands          | Served  |
| -------------------------- | -------------------- | ----------------------------- | ------- |
| `/autocomplete/businesses` | `BusinessSuggestion` | people, addresses, liens      | yes     |
| `/autocomplete/people`     | `PersonSuggestion`   | businesses, addresses, liens  | not yet |
| `/autocomplete/addresses`  | `AddressSuggestion`  | businesses, people            | not yet |
| `/autocomplete/liens`      | `LienSuggestion`     | businesses, people, addresses | not yet |
...
// The same as client.suggest({ q: "howard concrete" }).
const businesses = await client.search("businesses", { q: "howard concrete" });

// Typed by route: the filters it takes, the relations it expands, its rows.
const people = await client.search("people", {
  q: "frank howard",
  include: ["businesses"],
  filters: { business: { state: ["PA"] }, address: { city: "Canonsburg" } },
});
